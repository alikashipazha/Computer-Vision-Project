...
def main():
    ...
    model = HeatmapUNet(in_channels=3, out_channels=4).to(device)
    ...
    checkpoint_path = 'checkpoints/corner_heat_best.pth'
    ...
    print("Starting heatmap regression (Approach B) training loop...")
    for epoch in range(start_epoch, epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            print("==> New best heatmap model saved!")
    ...
    plt.xlabel('Epochs')
    plt.ylabel('MSE Heatmap Loss')
    plt.title('Approach B (Heatmap Regression) - Training Loss Curves')
    plt.legend()
    plt.grid(True)
    plt.savefig('docs/corner_heatmap_loss.png')
